...
resource "azuread_group_member" "education" {
  for_each = {
    for user in local.users : user.first_name => user if user.job_title == "Education"
  }
...
resource "azuread_group_member" "managers" {
 for_each = {
    ...
    user.first_name => user if user.job_title == "Managers"
  }
...
resource "azuread_group_member" "engineers" {
  for_each = {
    for user in local.users :
    user.first_name => user if user.job_title == "Engineer"
  }
